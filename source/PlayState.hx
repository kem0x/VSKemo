...
class PlayState extends MusicBeatState
{
	...
	private var doTheFunnyCamera:Bool;
	...
	override public function create()
	{
		...
		healthBar.createFilledBar(0xFF999999, 0xFF66FF33);
		// healthBar
		add(healthBar);
	...
	function switchEnemyAndStage(enemy:String, stageName:String)
	{
		healthBarBG.visible = false;
		kadeEngineWatermark.visible = false;
		healthBar.visible = false;
		...
		sys.thread.Thread.create(() ->
		{
			...
			remove(boyfriend);
			remove(dad);
			dad = new Character(100, 100, enemy);
			...
			if (stageName != "")
			{
				...
				if (stageName != "matrix")
				{
					for (layer in Stage.toAdd)
					{
						layer.shader = new KemoShader();
					}
				}
			}
	...
	override function stepHit()
	{
		...
		if (SONG.song == "Leet")
		{
			switch (curStep)
			{
				case 636:
					{
						doTheFunnyCamera = true;
						Stage.camZoom = 1.2;
						FlxG.camera.follow(new FlxObject(dad.getMidpoint().x, dad.getMidpoint().y - 80, 1, 1), LOCKON);
						dad.playAnim('snap', true);
					}

				case 642:
					{
						// switchEnemyAndStage("shaggy", "shaggy");
						// switchEnemyAndStage("matt", "gym");
						camZooming = true;
						switchEnemyAndStage("pico", "philly");
					}

				case 1145:
					{
						Stage.camZoom = 1.5;
						FlxG.camera.follow(new FlxObject(dad.getMidpoint().x, dad.getMidpoint().y - 50, 1, 1), LOCKON);
						dad.playAnim('snap', true);
					}

				case 1154:
					{
						switchEnemyAndStage("spooky", "halloween");
					}
				...
				case 1787:
					{
						switchEnemyAndStage("bf-swag", "stage");
					}
				...
				case 2163:
					{
						...
						dad.shader = wiggleEffect.shader;
					}

				case 2177:
					{
						dad.shader = boyfriend.shader;
					}

				case 2208:
					{
						effectSprite = new FlxEffectSprite(dad);
						effectSprite.y += 350;

						var waveEf = new FlxWaveEffect(FlxWaveMode.ALL);
						waveEf.direction = FlxWaveDirection.HORIZONTAL;

						effectSprite.effects = [waveEf];
				...
				case 2287:
					{
						remove(Stage.toAdd[0]);

						dad.shader = boyfriend.shader;

						effectSprite.effects = [new FlxGlitchEffect(10, 2, 0.1)];
				...
				case 2342:
					{
						doTheFunnyCamera = false;

						var tx = new FlxText(0, 0);
						tx.size = 50;
						tx.screenCenter();
						tx.scrollFactor.set();
						tx.y = FlxG.height * -1.5;
						tx.cameras = [camHUD];
						tx.text = "I WILL BE BACK..";
						add(tx);

						FlxTween.tween(tx, {y: FlxG.height / 2}, 1.6, {ease: FlxEase.expoIn, startDelay: 0.4});
						FlxTween.tween(FlxG.camera, {y: FlxG.height}, 1.6, {ease: FlxEase.expoIn, startDelay: 0.4});
						FlxG.save.data.leeted = true;
						FlxG.save.flush();
					}
			}
		}
		else if (SONG.song == "Copycat")
		{
			switch (curStep)
			{
				case 339:
				...
				case 1405:
					{
						switchEnemyAndStage("shaggy", "halloween");
					}
				...
				case 2166:
					{
						switchEnemyAndStage("kemo", "matrix");
					}
			}
		}
	...
	override function beatHit()
	{
		...
		if (currentSection != null)
		{
			...
			if ((!dad.animation.curAnim.name.startsWith("sing"))
				&& (!dad.animation.curAnim.name.startsWith("snap"))
				&& dad.curCharacter != 'gf')
		...
		if (!endingSong && currentSection != null)
		{
			...
			if (!PlayStateChangeables.Optimize)
				switch (Stage.curStage)
				{
					...
					case 'gym':
						{
							if (dad.curCharacter == "matt" || dad.curCharacter == "kemo")
							{
